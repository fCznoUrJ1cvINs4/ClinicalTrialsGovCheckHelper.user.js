...
(($, undefined)=>{
	$(()=>{
		...
		// 結果Tableコンテナ (ダイアログ右側)
		const resultContainer = GM_addElement(
			container,
			"div",
			{
			...
			Promise.all(TaskList).then((DataList)=>{
				DataList.sort((d1,d2) => d1.protocolSection.statusModule.lastUpdateSubmitDate < d2.protocolSection.statusModule.lastUpdateSubmitDate ? 1 : -1).forEach(data=> {
					if(data) {
						...
						GM_addElement(
							GM_addElement(tableRow,"td", {style:"border: 1px solid black;text-align:center;white-space: pre;padding: .3rem 1rem;"}),
							"a",{href:`/study/${i.nctId ?? ""}`,textContent:i.nctId ?? ""}
						);
						...
						GM_addElement(tableRow,"td", {style:"border: 1px solid black;text-align:center;white-space: pre;padding: .3rem 1rem;", textContent:`${s.primaryCompletionDateStruct.date ?? ""}\n(${s.primaryCompletionDateStruct.type ?? "-"})`});
						GM_addElement(tableRow,"td", {style:"border: 1px solid black;text-align:center;white-space: pre;padding: .3rem 1rem;", textContent:`${s.completionDateStruct.date ?? ""}\n(${s.completionDateStruct.type ?? "-"})`});
		...
		// ダイアログ表示 ホームバナー(リンク先が"/"の"a"要素)をを右クリック
